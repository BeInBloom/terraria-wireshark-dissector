local player_id = ProtoField.uint8("terraria.player_hurt_v2.player_id", "Player ID", base.DEC)
local death_reason = ProtoField.bytes("terraria.player_hurt_v2.death_reason", "Death Reason")
local death_reason_flags = ProtoField.uint8(
	"terraria.player_hurt_v2.death_reason_flags",
	"Death Reason Flags",
	base.HEX
)
local killer_player_id = ProtoField.int16(
	"terraria.player_hurt_v2.killer_player_id",
	"Killer Player ID",
	base.DEC
)
local killer_npc_index = ProtoField.int16(
	"terraria.player_hurt_v2.killer_npc_index",
	"Killer NPC Index",
	base.DEC
)
local projectile_index = ProtoField.int16(
	"terraria.player_hurt_v2.projectile_index",
	"Projectile Index",
	base.DEC
)
local other_death_type = ProtoField.uint8(
	"terraria.player_hurt_v2.other_death_type",
	"Other Death Type",
	base.DEC
)
local projectile_type = ProtoField.int16(
	"terraria.player_hurt_v2.projectile_type",
	"Projectile Type",
	base.DEC
)
local item_type = ProtoField.int16(
	"terraria.player_hurt_v2.item_type",
	"Item Type",
	base.DEC
)
local item_prefix = ProtoField.uint8(
	"terraria.player_hurt_v2.item_prefix",
	"Item Prefix",
	base.DEC
)
local custom_reason = ProtoField.string(
	"terraria.player_hurt_v2.custom_reason",
	"Death Reason Text"
)
local damage = ProtoField.int16("terraria.player_hurt_v2.damage", "Damage", base.DEC)
local hit_direction = ProtoField.uint8(
	"terraria.player_hurt_v2.hit_direction",
	"Hit Direction",
	base.DEC
)
local flags = ProtoField.uint8("terraria.player_hurt_v2.flags", "Flags", base.HEX)
local cooldown_counter = ProtoField.int8(
	"terraria.player_hurt_v2.cooldown_counter",
	"Cooldown Counter",
	base.DEC
)
...
local function build(payload)
	...
	payload:uint8(flags)
	payload:sbyte(cooldown_counter)
end
